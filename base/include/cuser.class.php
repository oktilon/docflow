...
class CUser {
    ...
    public $middle_name = '';
    public $email = '';
    public $double_field_email = '';
    public $phone = '';
    public $double_field_phone = '';
    public $whatsapp = '';
    public $gphone = '';
    ...
    public function __construct($arg = 0) {
        ...
        if(is_array($arg) || is_object($arg)) {
            foreach($arg as $key => $val) {
                ...
                $this->$k = self::getProperty($key, $val);
            }
            if($readAcl) $this->readAcl();
            //if($readFrm) $this->readAcl();
        }
    }
    ...
    public static function getProperty($k, $v) {
        switch($k) {
            ...
            case 'date_create':
            case 'last_reg': return new DateTime($v);
            case 'telegram_id':
            case 'is_blocked':
            case 'lette_reg':
            case 'admin_id':
            ...
            case 'frm':return self::parseIds($v);
            case 'rght':return self::parseIds($v);
        }
        return $v;
    }

    public static function parseIds($str) {
        $r = [];
        $a = explode(',', $str);
        foreach($a as $s) {
            $r[] = intval($s);
